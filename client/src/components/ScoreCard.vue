<script setup lang="ts">
  ...
  import { lower_section_keys, slots, sum_upper, total_lower, total_upper, type SlotKey } from 'domain/src/model/yahtzee.score';
...
import { score } from 'domain/src/model/yahtzee.slots';
  ...
  const playerScores = (key: SlotKey): { player: string; score: number | undefined }[] => {
    return players.value.map((player, i) => ({player, score: slot_score(game.scores[i], key)}))
  }
      ...
      <table class="scorecard">
        <tbody>
          ...
          <tr>
            <td>Sum</td>
            <td>63</td>
            <td v-for="(player, index) in players" :class="activeClass(player)">{{sum_upper(game.scores[index].upper_section.scores)}}</td>
          </tr>
          <tr>
            <td>Bonus</td>
            <td>50</td>
            <td v-for="(player, index) in players" :class="activeClass(player)">{{game.scores[index].upper_section.bonus}}</td>
          </tr>
          <tr>
            <td>Total</td>
            <td></td>
            <td v-for="(player, index) in players" :class="activeClass(player)">{{total_upper(game.scores[index].upper_section)}}</td>
          </tr>
